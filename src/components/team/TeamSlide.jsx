...
function TeamSlide() {
  ...
  return (
    <div className="py-15 bg-red-500 w-full
    2xl:py-30 xl:py-30 lg:py-20 md:py-20 sm:py-20
    ">
      <div className=" mx-auto flex-col sm:flex-row gap-8 items-center
      2xl:flex xl:flex lg:flex md:flex sm:flex bg-pink-500">
        {/* Left Section */}
        <div className="sm:w-1/3  flex-col rounded-lg  2xl:w-[541px] 2xl:max-h-[418px] overflow-hidden team-out">
          <h1 className="text-yellow-600 font-[Poppins] font-bold
           2xl:text-2xl xl:text-2xl lg:text-lg
           md:text-lg sm:text-lg text-lg 2xl:mb-4 xl:mb-4 lg:mb-4 md:mb-0 sm:mb-4 mb-4
          ...
          <h1 className="font-[Poppins] font-normal 2xl:text-[48px]
           ...
           lg:text-[40px] lg:leading-snug
           md:text-[32px] md:leading-snug
           sm:text-[32px] sm:leading-snug
           text-xs leading-snug">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Aperiam minima nam.
          </h1>
          <div className="flex  2xl:my-6 xl:my-6 lg:my-4 md:my-4 sm:my-4 my-2">
            <button
        ...
        2xl:h-[498px] xl:h-[498px] lg:h-[400px] md:h-[400px] h-[498px]
         pt-8  bg-green-500">
          <div
            className="flex transition-transform duration-700 
            2xl:[transform:translateX(calc(-50%*var(--index)))]
            xl:[transform:translateX(calc(-70.5%*var(--index)))]
            lg:[transform:translateX(calc(-100%*var(--index)))]
            md:[transform:translateX(calc(-83%*var(--index)))]
            sm:[transform:translateX(calc(-100%*var(--index)))]
            " style={{ "--index": index }}
          ...
          >
            {list.map((member, i) => (
              <div
                key={i}
                className="2xl:w-1/2 xl:w-2/3 flex-shrink-0 xl:mr-6 2xl:mr-0 md:mr-5"
              >
                <div className="bg-[#001A3B] text-white rounded-2xl p-6 
                2xl:h-[434px] xl:h-[434px] lg:h-[350px] md:h-[330px] sm:h-[400px] h-[434px]
                2xl:w-[405px] xl:w-[405px] lg:w-[350px] md:w-[301px] sm:w-[400px] w-[405px] 
                flex flex-col text-center justify-end">
                  <div className="bg-white h-[373px] rounded-xl mb-4 overflow-hidden">
                    <img
                      src={member.image}
                      alt={member.name}
                  ...
                  <h4 className="font-bold font-[Poppins] text-[24px] font-bold">{member.name}</h4>
                  <p className="text-gray-400 font-[Poppins] text-[16px] font-bold">{member.title_name}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={prevSlide}
            className="absolute left-0 top-1/2 -translate-y-1/2 bg-white border rounded-full p-2 w-[60px] h-[60px] justify-center items-center flex" 
          >
            <FaChevronLeft className="w-[24px] h-[24px]"  />
          </button>
          <button
            onClick={nextSlide}
            className="absolute right-0 top-1/2 -translate-y-1/2 bg-white border rounded-full p-2 w-[60px] h-[60px] justify-center items-center flex" 
          >
